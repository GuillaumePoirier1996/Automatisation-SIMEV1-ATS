...
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-1982 Clés 37.3 et 38.3
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($break2 -eq $False) {
				...
				# Protection type ZEP
				# Ts pour le controle optique ZEP pour l'image TCOi
				[String] $TsProtectionTCOi = "SY_ZEP_" + $Protection + "_STATUS"
				$TsList += $TsProtectionTCOi
				...
				[String] $ProtTcoiActiveState = "KEY_37_3_SET"
				[String] $ProtTcoiActiveValue = "2"
				[String] $ProtTcoiInactiveState = "NONE"
				[String] $ProtTcoiInactiveValue = "0"
				
				# Variable necessaire pour l'avertissement 3136
				[String] $AvertId = "3136"
	
				$ItiAlarm = $Instance
				$ItiAlarm = $ItiAlarm.replace(" par ","_")
				[String] $ItiAlarm = $ItiAlarm.replace("-","")
				
				[String] $AvertLabel = "\""C_CCK_MCMD_ITI_" + $ItiAlarm + "_FORMER_DA\"""
...
### CLE 37.3				
				...
				WriteStepPres -numStep $numStep -displayStep "CLE 37.3" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Ecriture de la cle
				[String] $Key373 = "37.3 " + $Protection
				WriteKey -Key $Key373 -i "1" -OutputScript $OutputScript -LogFullPath $LogFullPath
				...
				WriteTSVerif -ts $TsProtectionTCOi -tsState $ProtTcoiActiveState -tsValue $ProtTcoiActiveValue -OutputScript $OutputScript -LogFullPath $LogFullPath
				...
				[String] $exp = "((keyTest1 == 0) && Verif$TsProtectionTCOi)"
...
### CLE 21				
				...
				# Ecriture du resultat et de la fin du step
				[String] $exp = "((keyTest == 0) && (confirmSetUpTest == 0) && Verif$TsFNITFNAU && testAvertVerif1)"
				WriteStepEnd -exp $exp -numStep $numStep -comment "Test realise automatiquement - Reserve tickets rappels" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 38.3				
				# Ecriture de la presentation du step
				WriteStepPres -numStep $numStep -displayStep "CLE 38.3" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Ecriture de la cle
				[String] $Key383 = "38.3 " + $Protection
				WriteKey -Key $Key383 -i "1" -OutputScript $OutputScript -LogFullPath $LogFullPath
				...
				# Ecriture de la vérification type Ts
				WriteTSVerif -ts $TsProtectionTCOi -tsState $ProtTcoiInactiveState -tsValue $ProtTcoiInactiveValue -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 24
				...
				# Ecriture de la cle
				WriteKeySettingUp -Key "24" -Instance $Instance -OutputScript $OutputScript -LogFullPath $LogFullPath -poste $Poste
