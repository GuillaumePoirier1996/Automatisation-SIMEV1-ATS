...
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-765 Clés 37.2, 38.4 et 38.2
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($Instance.Substring(0,4) -eq "ITI_") {
				# Instance pour la clé 21
				$Instance = $Instance.Split("_")
				$Instance = $Instance[1]
				
				# Instance pour les autres clés
				$itiAu = $Instance.Replace(" par ","/")
				[String] $itiAu = $itiAu.Replace("-"," ")
			}
			...
			if ($break2 -eq $False) {
				...
				# Variable necessaire pour l'alarme 3136
				[String] $AvertId = "3136"
...
### CLE 37.2				
				...
				WriteStepPres -numStep $numStep -displayStep "CLE 37.2" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Ecriture de la cle
				[String] $Key372 = "37.2 " + $itiAu
				WriteKey -Key $Key372 -i "1" -OutputScript $OutputScript -LogFullPath $LogFullPath
				...
				WriteStepEnd -exp "(keyTest1 == 0)" -numStep $numStep -comment "Test realise automatiquement - Reserve tickets rappels" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 21				
				...
				# Ecriture du resultat et de la fin du step
				WriteStepEnd -exp "((keyTest == 0) && (confirmSetUpTest == 0) && Verif$TsFNITFNAU && testAvertVerif1)" -numStep $numStep -comment "Test realise automatiquement" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 38.4				
				...
				# Ecriture de la presentation du step
				WriteStepPres -numStep $numStep -displayStep "CLE 38.4" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Ecriture de la cle
				[String] $Key384 = "38.4 " + $itiAu
				WriteKey -Key $Key384 -i "1" -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 38.2
				# Ecriture de la presentation du step
				WriteStepPres -numStep $numStep -displayStep "CLE 38.2" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Ecriture de la cle
				[String] $Key382 = "38.2 " + $itiAu
				WriteKey -Key $Key382 -i "1" -OutputScript $OutputScript -LogFullPath $LogFullPath
