...
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-1739 : Itinéraires / Autorisations 
#					compatibles
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($break2 -eq $False) {
				...
				# Creation de la liste des itinéraires ou autorisations compatibles
				$ItiAuComp = @()
				$TsFnitItiAuComp = @()

				# CO FNIT / FNAU / FNIT PRS à la position 2
				...
				# Boucle pour les itinéraires compatibles à partir de la position 4
				[Bool] $BreakS = $FALSE
				
				for ([Int] $p = 3; $p -lt $Elements.Count -and $BreakS -eq $FALSE; $p++) {
					# Element testé
					...
					$ItiAuComp += $Elements[$p]
					
					[Int] $fnitCompStart = $p + 1
				}
				
				# Boucle pour les FNIT / FNAU pour les itinéraires compatibles à partir de la fin des itinéraires / autorisations compatibles
				for ([Int] $p = $fnitCompStart; $p -lt $Elements.Count; $p++) {
					switch -Wildcard ($Elements[$p]) {
						("*FNIT*") {
							# Ts pour le controle optique FNIT PRS
							$fnitFnauComp = $Elements[$p]
							if ($fnitFnauComp.Substring(0,8) -eq "FNIT_PRS") {
								$fnitFnauComp = $fnitFnauComp.Split("_")
								$fnitFnauComp = $fnitFnauComp[$fnitFnauComp.Length - 1]
								$TsFnitItiAuComp += "S_CCK_MFCO_SIG_" + $fnitFnauComp + "_FNIT_PRS_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_SIG_" + $fnitFnauComp + "_FNIT_PRS_ASPECT_CTRL"
							}
							# Ts pour le controle optique FNIT
							else {
								$fnitFnauComp = $fnitFnauComp.Split("_")
								$fnitFnauComp = $fnitFnauComp[$fnitFnauComp.Length - 1]
								$TsFnitItiAuComp += "S_CCK_MFCO_SIG_" + $fnitFnauComp + "_FNIT_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_SIG_" + $fnitFnauComp + "_FNIT_ASPECT_CTRL"
							}
						}
						("*FNAU*") {
							# Ts pour le controle optique FNAU2
							$fnitFnauComp = $Elements[$p]
							if ($fnitFnauComp.Substring(0,6) -eq "FNAU2_") {
								$fnitFnauComp = $fnitFnauComp.Split("_")
								$fnitFnauComp = $fnitFnauComp[$fnitFnauComp.Length - 1]
								$TsFnitItiAuComp += "S_CCK_MFCO_FNAU_" + $fnitFnauComp + "_FNAU2_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_FNAU_" + $fnitFnauComp + "_FNAU2_ASPECT_CTRL"
							}
							# Ts pour le controle optique FNAU
							else {
								$fnitFnauComp = $fnitFnauComp.Split("_")
								$fnitFnauComp = $fnitFnauComp[$fnitFnauComp.Length - 1]
								$TsFnitItiAuComp += "S_CCK_MFCO_FNAU_" + $fnitFnauComp + "_FNAU_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_FNAU_" + $fnitFnauComp + "_FNAU_ASPECT_CTRL"
							}
						}
					}
				}
...
### CLE 21				
				...
				[String] $exp = "((keyTest == 0) && (confirmSetUpTest == 0) && Verif$TsFNITFNAU)"
...
### boucle for pour parcourir tous les  itinéraires ou autorisations compatibles
				
				[Bool] $Break3 = $FALSE
				
				for ([Int] $m=0; $m -lt $ItiAuComp.Count -and $Break3 -eq $FALSE;$m++) {
					
					# Comparaison a un itineraire de la 2D4 pour les itineraires compatibles
					For ([Int] $z = 0; $z -lt $global:ItiDestSpeTerms.Count; $z++) {
						if ($ItiAuComp[$m] -eq $global:ItiDestSpeTerms[$z]."Itineraire") {
						# Condition d'ecriture dans le mananger et du script specifique
						$Break3 = $True
						# Arret de la boucle for
						break
...
### CLE 21				
					if ($Break3 -eq $FALSE) {
						...
						# Ecriture de la cle
						WriteKeySettingUp -Key "21" -Instance $ItiAuComp[$m] -OutputScript $OutputScript -LogFullPath $LogFullPath -poste $Poste
						...
						# Ecriture de la vérification type Ts
						WriteTSVerif -ts $TsFnitItiAuComp[$m] -tsState $TsItiAuActiveState -tsValue $TsItiAuActiveValue -OutputScript $OutputScript -LogFullPath $LogFullPath
						...
						# Ecriture du resultat et de la fin du step
						[String] $exp = "((keyTest == 0) && (confirmSetUpTest == 0) && Verif" + $TsFnitItiAuComp[$m] + ")"
						WriteStepEnd -exp $exp -numStep $numStep -comment "Test realise automatiquement" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath				
...
### CLE 24
						...
						# Ecriture de la cle
						WriteKeyDest -Key "24" -Instance $ItiAuComp[$m] -OutputScript $OutputScript -LogFullPath $LogFullPath
						...
						# Ecriture de la vérification type Ts
						WriteTSVerif -ts $TsFnitItiAuComp[$m] -tsState $TsItiAuInactiveState -tsValue $TsItiAuInactiveValue -OutputScript $OutputScript -LogFullPath $LogFullPath
						...
						# Ecriture du resultat et de la fin du step
						[String] $exp = "((keyTest == 0) && Verif" + $TsFnitItiAuComp[$m] + ")"
						WriteStepEnd -exp $exp -numStep $numStep -comment "Test realise automatiquement" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
					...
					if ($Break3 -eq $TRUE) {
						# Information d'un itinéraires descoppé
						(	("		println(""Itinéraire ou autorisation " + $ItiAuComp[$m] + " specifique non pris en compte"");"),
							("")
						) >> $OutputScript
						$numStep = $numStep + 2
					}
