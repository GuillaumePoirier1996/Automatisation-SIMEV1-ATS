...
#
# File			:	Generateur_TST_PSO_1736
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-1736 Clé 31
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($break2 -eq $False) {
				...
				if ($TestCOIN -eq $FALSE) {
					$Protection = $InstanceCell.Split("_")
					$Protection = $Protection[$Protection.Length - 1]
					
					if ($Protection.Substring(0,3) -eq "ZEP") {
						# Protection type ZEP
						$Protectiontest = $Protection.Substring(3,$Protection.Length - 3)
						# Ts pour le controle optique ZEP
						[String] $TsProtection = "S_CCK_MFCO_ZEP_" + $Protectiontest + "_ZEP_LABEL_ASPECT_CTRL"
						$TsList += "S_CCK_MFCO_ZEP_" + $Protectiontest + "_ZEP_LABEL_ASPECT_CTRL"
					}
					
					if ($Protection.Substring(0,3) -eq "SEL") {
						# Protection type SEL
						$Protectiontest = $Protection.Substring(3,$Protection.Length - 3)
						# Ts pour le controle optique SEL
						[String] $TsProtection = "S_CCK_MFCO_SEL_" + $Protectiontest + "_SEL_LABEL_ASPECT_CTRL"
						$TsList += "S_CCK_MFCO_SEL_" + $Protectiontest + "_SEL_LABEL_ASPECT_CTRL"
					}
					...
					# Variable necessaire pour l'alarme 3528
					[String] $AlarmId1 = "3528"
					$ItiAlarm = $Instance.Replace(" par ","/")
					[String] $AlarmLabel1 = "[\""$ItiAlarm\""]"
					
					# Variable necessaire pour l'alarme 3545
					[String] $AlarmId2 = "3545"
					[String] $AlarmLabel2 = $AlarmLabel1
...
### ETAPE POUR LES BESOINS DE L'AUTOMATISATION
					
					# LES ETATS D'ORGANES DISPARAISSENT S'ILS NE CHANGENT PAS LORSQUE LES FICHIERS DE LOGS DU COEUR CK CHANGENT
					# POUR CONTOURNER CETTE PROBLEMATIQUE, DEUX ACTION D'ORGANES SONT EFFECTUEES CO -> ACTIF PUIS CO -> INACTIF
					
					# CONTROLE DU CO A ACTIF
					WriteOrganeManualAction -organName $CO -command "CONTROL_STATE_TO_ACTIVE" -display "COMMANDE DE $CO A L ETAT ACTIF" -OutputScript $OutputScript -LogFullPath $LogFullPath
					...
					# CONTROLE DU CO A INACTIF
					WriteOrganeManualAction -organName $CO -command "CONTROL_STATE_TO_INACTIVE" -display "COMMANDE DE $CO A L ETAT INACTIF" -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 31				
					...
					# Ecriture de la cle
					[String] $Key31 = "31 " + $Protectiontest
...
### CLE 21
					...
					# Ajustement du delai et prise de l'horodatage de la clé et horodatage pour le test de l'organe CO
					(	("		date hKey = hKey.now();"),
					...
					WriteLogVerifBeforeAction -organName $CO -organState "0" -h "hCo1" -OutputScript $OutputScript -LogFullPath $LogFullPath
					...
					# Ecriture du resultat et de la fin du step
					[String] $exp = "((keyTest == 0) && (confirmSetUpTest == 0) && Verif$TsFNITFNAU && Verif$CO && testAlarmVerif1)"
					WriteStepEnd -exp $exp -numStep $numStep -comment "Test realise automatiquement" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 32				
					...
					# Ecriture de la presentation du step
					WriteStepPres -numStep $numStep -displayStep "CLE 32" -OutputScript $OutputScript -LogFullPath $LogFullPath
					
					# Ecriture de la cle
					[String] $Key32 = "32 " + $Protectiontest
					...
					# Ecriture du resultat et de la fin du step
					[String] $exp = "((keyTest1 == 0) && Verif$TsProtection && testAlarmVerif1)"
					WriteStepEnd -exp $exp -numStep $numStep -comment "Test realise automatiquement" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CLE 32 ET 33
					...
					[String] $Key33 = "33 " + $Protectiontest
