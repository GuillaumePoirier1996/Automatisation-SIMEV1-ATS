...
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-4374 : Itinéraires / Autorisations 
#					incompatibles
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($break2 -eq $False) {
				...
				# Creation de la liste des itinéraires ou autorisations incompatibles
				$ItiAuIncomp = @()
				$TsFnitItiAuIncomp = @()
				
				# Organe EIT / EAU à la position 2
				$EitEau = $Elements[2].split("(")
				$EitEau = $EitEau[$EitEau.Length - 1]
				$EitEau = $EitEau.replace(")","")
				$OrganList += $EitEau

				# CO FNIT / FNAU / FNIT PRS à la position 3
				switch -Wildcard ($Elements[3]) {
					("*FNIT*") {
				...
				# Boucle pour les itinéraires incompatibles à partir de la position 4
				[Bool] $BreakS = $FALSE
				
				for ([Int] $p = 4; $p -lt $Elements.Count -and $BreakS -eq $FALSE; $p++) {
					# Element testé
					[String] $testElem = $Elements[$p + 1]
					
					# Test booléen pour l'élément suivant : on s'arrête au prochain indice si l'élément commence par FNIT ou FNAU
					$BreakS = (($testElem.Substring(0,4) -eq "FNIT") -or ($testElem.Substring(0,4) -eq "FNAU"))
							
					# Itinéraire / Autorisation Compatible
					$ItiAuIncomp += $Elements[$p]
					
					[Int] $fnitIncompStart = $p + 1
				}
				
				# Boucle pour les FNIT / FNAU pour les itinéraires incompatibles à partir de la fin des itinéraires / autorisations compatibles
				for ([Int] $p = $fnitIncompStart; $p -lt $Elements.Count; $p++) {
					switch -Wildcard ($Elements[$p]) {
						("*FNIT*") {
							# Ts pour le controle optique FNIT PRS
							$fnitFnauIncomp = $Elements[$p]
							if ($fnitFnauIncomp.Substring(0,8) -eq "FNIT_PRS") {
								$fnitFnauIncomp = $fnitFnauIncomp.Split("_")
								$fnitFnauIncomp = $fnitFnauIncomp[$fnitFnauIncomp.Length - 1]
								$TsFnitItiAuIncomp += "S_CCK_MFCO_SIG_" + $fnitFnauIncomp + "_FNIT_PRS_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_SIG_" + $fnitFnauIncomp + "_FNIT_PRS_ASPECT_CTRL"
							}
							# Ts pour le controle optique FNIT
							else {
								$fnitFnauIncomp = $fnitFnauIncomp.Split("_")
								$fnitFnauIncomp = $fnitFnauIncomp[$fnitFnauIncomp.Length - 1]
								$TsFnitItiAuIncomp += "S_CCK_MFCO_SIG_" + $fnitFnauIncomp + "_FNIT_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_SIG_" + $fnitFnauIncomp + "_FNIT_ASPECT_CTRL"
							}
						}
						("*FNAU*") {
							# Ts pour le controle optique FNAU2
							$fnitFnauComp = $Elements[$p]
							if ($fnitFnauIncomp.Substring(0,6) -eq "FNAU2_") {
								$fnitFnauIncomp = $fnitFnauIncomp.Split("_")
								$fnitFnauIncomp = $fnitFnauIncomp[$fnitFnauIncomp.Length - 1]
								$TsFnitItiAuIncomp += "S_CCK_MFCO_FNAU_" + $fnitFnauIncomp + "_FNAU2_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_FNAU_" + $fnitFnauIncomp + "_FNAU2_ASPECT_CTRL"
							}
							# Ts pour le controle optique FNAU
							else {
								$fnitFnauIncomp = $fnitFnauIncomp.Split("_")
								$fnitFnauIncomp = $fnitFnauIncomp[$fnitFnauIncomp.Length - 1]
								$TsFnitItiAuIncomp += "S_CCK_MFCO_FNAU_" + $fnitFnauIncomp + "_FNAU_ASPECT_CTRL"
								$TsList += "S_CCK_MFCO_FNAU_" + $fnitFnauIncomp + "_FNAU_ASPECT_CTRL"
							}
						}
					}
				}
...
### EIT EAU INACTIF, BLOQUAGE ET CLE 21				

				# Ecriture de la presentation du step
				WriteStepPres -numStep $numStep -displayStep "COMMANDE DE L'ORGANE $EitEau A INACTIF, BLOQUAGE ET CLE 21" -OutputScript $OutputScript -LogFullPath $LogFullPath
				...
				[String] $exp = "((keyTest == 0) && (confirmSetUpTest == 0) && Verif$TsFNITFNAU && testAlarmVerif1)"
				WriteStepEnd -exp $exp -numStep $numStep -comment "Test realise automatiquement - Reserve tickets rappels" -numTest $numTest -OutputScript $OutputScript -LogFullPath $LogFullPath
					
				$numStep++

### boucle for pour parcourir tous les itinéraires ou autorisations incompatibles
				
				for ([Int] $m=0; $m -lt $ItiAuIncomp.Count;$m++) {
...
### CLE 21				
					...
					# Ecriture de la cle
					WriteKeySettingUp -Key "21" -Instance $ItiAuIncomp[$m] -OutputScript $OutputScript -LogFullPath $LogFullPath -poste $Poste
					...
					# Ecriture de la vérification type Ts
					WriteTSVerif -ts $TsFnitItiAuIncomp[$m] -tsState $TsItiAuInactiveState -tsValue $TsItiAuInactiveValue -OutputScript $OutputScript -LogFullPath $LogFullPath
					
					# Variable necessaire pour l'alarme 3522
					[String] $AlarmId = "3522"
					$ItiAlarm = $ItiAuIncomp[$m].Replace(" par ","/")
					[String] $AlarmLabel = "[\""$ItiAlarm\""]"
					...
					# Ecriture du resultat et de la fin du step
					[String] $exp = "((keyTest == 0) && (confirmSetUpTest == 0) && testAlarmVerif1 && Verif" + $TsFnitItiAuIncomp[$m] + ")"
