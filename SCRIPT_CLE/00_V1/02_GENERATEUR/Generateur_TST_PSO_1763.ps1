...
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-1763 Clé 21
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($break2 -eq $False) {
				...
				# TK CAG et Organe à la position 0
				$tkCag = $Elements[0].Split("(")
				$tkCag = $tkCag[$tkCag.length - 1]
				$tkCag = $tkCag.Replace(")","")
				$OrganList += $tkCag
				
				# CO FNIT / FNAU / FNIT PRS à la position 1
				switch -Wildcard ($Elements[1]) {
					("*FNIT*") {
				...
				# Aiguille à manipuler à la position 2
				$Aig = $Elements[2]
				
				# TC CAG et Organe à la position 5
				$tcCag = $Elements[5].Split("(")
				$tcCag = $tcCag[$tcCag.length - 1]
				$tcCag = $tcCag.Replace(")","")
				$OrganList += $tcCag
				...
				[String] $AlarmId = "3502"
				[String] $AlarmLabel = "[\""$tcCag\""]"
				
				# Position de l'aiguille d'après la PT2A
				$posPT2A = $InstanceCell.Substring($InstanceCell.Length - 1, 1)
				Switch ($posPT2A) {
					("G") {
						# Position gauche
						$tkCagPt2aState = "0"
						$tkCagInvPt2aState = "1"
						# Commande de l'organe CAG
						$cmdCagPt2a = "CONTROL_STATE_TO_INACTIVE"
						$cmdCagInvPt2a = "CONTROL_STATE_TO_ACTIVE"
					}
					("D") {
						# Position droite
						$tkCagPt2aState = "1"
						$tkCagInvPt2aState = "0"
						# Commande de l'organe CAG
						$cmdCagPt2a = "CONTROL_STATE_TO_ACTIVE"
						$cmdCagInvPt2a = "CONTROL_STATE_TO_INACTIVE"
					}
					("N") {
						# Position normale
						$tkCagPt2aState = "1"
...
### CAG COMMANDE DANS LA POSITION CONTRAIRE A LA PT2A ET BLOQUAGE			

				# Ecriture de la presentation du step
				WriteStepPres -numStep $numStep -displayStep "COMMANDE DE L'ORGANE $tkCag DANS LA POSITION CONTRAIRE A LA PT2A ET BLOQUAGE" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Commande de l'organe CAG
				WriteOrganeManualAction -organName $tkCag -command $cmdCagInvPt2a -display "COMMANDE DE L'ORGANE $tkCag DANS LA POSITION CONTRAIRE A LA PT2A" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Bloquage du CAG
				WriteOrganeManualAction -organName $tkCag -command "BLOCK" -display "BLOQUAGE DE L'ORGANE $tkCag" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Ajustement du delai et prise de l'horodatage pour le test de l'organe CAG
					(	(""),
						("		date ho = ho.now();"),
				...
				WriteLogVerifBeforeAction -organName $tkCag -organState $tkCagInvPt2aState -h "ho1" -OutputScript $OutputScript -LogFullPath $LogFullPath
...
### CAG DEBLOQUAGE ET COMMANDE DANS LA POSITION DE LA PT2A ET BLOQUAGE

				# Ecriture de la presentation du step
				WriteStepPres -numStep $numStep -displayStep "DEBLOQUAGE DE L'ORGANE $tkCag ET COMMANDE DANS LA POSITION DE LA PT2A" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Debloquage du CAG
				WriteOrganeManualAction -organName $tkCag -command "UNBLOCK" -display "DEBLOQUAGE DE L'ORGANE $tkCag" -OutputScript $OutputScript -LogFullPath $LogFullPath
				
				# Commande de l'organe CAG
				WriteOrganeManualAction -organName $tkCag -command $cmdCagPt2a -display "COMMANDE DE L'ORGANE $tkCag DANS LA POSITION DE LA PT2A" -OutputScript $OutputScript -LogFullPath $LogFullPath
				...
				# Ecriture de la vérification type log CCK
				WriteLogVerifBeforeAction -organName $tkCag -organState $tkCagPt2aState -h "ho1" -OutputScript $OutputScript -LogFullPath $LogFullPath
