...
# Version 		: 	1.0
# Description 	: 	Generateur de la famille de test NExTEO_ATS_SFE_TST_PSO-1737 Clé 31
#
	...
	For ([Int] $i = 2; $worksheet.Cells.Item($i,1).Value() -ne $null -and $break1 -eq $FALSE; $i++) {
		if ($worksheet.Cells.Item($i,1).Value() -eq ("NExTEO_ATS_SFE_TST_PSO-" + $TestGenere)) {
			...
			if ($break2 -eq $False) {
				...
				[String] $ProtInactiveState = "GRIS_DE_PAYNE"
				[String] $ProtInactiveValue = "1"
...
### CLE 31				
				...
				# Ecriture de la vérification type Ts
				WriteTSVerif -ts $TsProtection -tsState $ProtInactiveState -tsValue $ProtInactiveValue -OutputScript $OutputScript -LogFullPath $LogFullPath
