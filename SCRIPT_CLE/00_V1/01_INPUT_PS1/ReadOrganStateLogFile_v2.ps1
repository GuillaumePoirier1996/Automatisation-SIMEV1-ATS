...
#
# Version 		: 	1.0
# Description 	: 	Recherche d'etat d'organe dans
#					les logs
#
...
try {
		...
		if(Test-Path -Path "$directory\$filenamePattern*") {
			'[INFO] - ' + $(Get-Date) + ' : ReadOrganStateLogFile_v2.ps1 : il existe des fichiers comportants la chaine de caractere demande' | Out-File -FilePath $LogExeFullPath -Append
		...
		if ($filename.Count -eq 0) {
			...
			'[ERROR] - ' + $(Get-Date) + ' : ReadOrganStateLogFile_v2.ps1 : impossible de trouver un fichier avec un titre composé de ' + $filenamePattern + ' dans le repertoire ' + $directory + ' pour l''organe ' + $organ_name | Out-File -FilePath $LogExeFullPath -Append
			$state_value ="4"
			exit $state_value	
		}
		...
		[String] $organ_line = Get-Content $logfile | Select-String -Pattern $organ_name | Select-String -Pattern "Nouvel état :" | sort -Descending | Select-Object -First 1
		
		if ($organ_line -eq "" -or $organ_line -eq $NULL) {
			# Gestion du cas ou l'organe est introuvable
			'[ERROR] - ' + $(Get-Date) + ' : ReadOrganStateLogFile_v2.ps1 : impossible de trouver l''organe ' + $organ_name + ' dans les fichiers proposes' | Out-File -FilePath $LogExeFullPath -Append
			$state_value ="5"
			exit $state_value
		}
		...
		if ($h -le $h_test) {
			...
			'[INFO] - ' + $(Get-Date) + ' : ReadOrganStateLogFile_v2.ps1 : le dernier etat de l''organe ' + $organ_name + ' est ' + $state_value | Out-File -FilePath $LogExeFullPath -Append
			exit $state_value
		}
		
		if ($state_value -eq "" -or $state_value -eq $NULL) {
			# Gestion du cas ou l'horodatage est incoherent
			'[ERROR] - ' + $(Get-Date) + ' : ReadOrganStateLogFile_v2.ps1 : horodatage incohérent avec l''heure du changement d''etat de l''organe' + $organ_name | Out-File -FilePath $LogExeFullPath -Append
			$state_value ="6"
			exit $state_value
		}
}
		...
		'[ERROR] - ' + $(Get-Date) + ' : ReadOrganStateLogFile_v2.ps1 : Erreur exeptionnelle pour l''organe ' + $organ_name + ' : ' + $($_.exception.message) | Out-File -FilePath $LogExeFullPath -Append
		$state_value = "7"
		exit $state_value
}